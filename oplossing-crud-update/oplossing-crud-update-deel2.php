<?php
	try {
		...
		if(isset($_POST["wijzigen"]) && isset($_POST["brewerNr"])) {
			$querySetStrings = array();

			foreach($brewersTableNames as $brewersTableName) {
				if($brewersTableName !== "brouwernr") {
					array_push($querySetStrings, $brewersTableName . "=:" . $brewersTableName);
				}
			}

			$updateBrewerQueryString = 	"UPDATE brouwers
										SET " . implode(", ", $querySetStrings) . "
										WHERE brouwernr = :brewerNr";
			...
			foreach($brewersTableNames as $brewersTableName) {
				if($brewersTableName !== "brouwernr") {
					$updateBrewerStatement->bindValue(":" . $brewersTableName, $_POST[$brewersTableName]);
				}
			}
	...
	<body>
		...
		<?php else: ?>
			<?php if(isset($brewer)): ?>
				...
				<form action="" method="post">
					<?php foreach($brewersTableNames as $brewersTableName): ?>
						<?php $brewersTableNameWithSpaces = str_replace("_", " ", $brewersTableName) ?>
						<?php if($brewersTableName !== "brouwernr"): ?>
							<label for="<?= $brewersTableName ?>">
								<?= $brewersTableNameWithSpaces === "brnaam" ? "brouwernaam" : $brewersTableNameWithSpaces ?>
							</label>
			...
			<?php if(isset($_GET["updateSuccess"])): ?>
				...
				<?php endif ?>
				<a href="oplossing-crud-update-deel2.php">OK</a>
				</div>
			<?php endif ?>
